...
A stepped walkthrough, built to be watched. Paste a JSON object of customer requests —
the key is a name or email, the value is what that person wrote — and each step
reveals the next:

| Step | Agent | What you see |
| --- | --- | --- |
| 1 · Requests | — | the raw JSON. **5**, **20** or **500 customers** loads a realistic mixed list |
| 2 · Grouped | Bucketing agent | every group, sized, with the requirements that bind it (most-shared first) |
| 3 · Products | Market research agent | the query sent to the web, and each candidate judged against the group |
| 4 · Suppliers | Sourcing agent | who could fulfil the pooled order |

**Linkup** does the searching in steps 3 and 4. Turn on *"search the live web with
Linkup"* and the agent writes a query from the group's binding requirements, searches
the real web, and every candidate keeps the source it came from — shown on the card.
Left off, it reads the local fixture catalogue: instant, deterministic, good for
rehearsal.

The samples are generated by
[`scripts/make_sample_customers.py`](scripts/make_sample_customers.py): monitors,
wearables and a few other categories, phrased a dozen different ways, with budgets
that follow the specs people ask for.

| Sample | Groups | Time |
| --- | --- | --- |
| 5 customers | 2 | instant |
| 20 customers | 7, the largest holding 6 | instant |
| 500 customers | 36, the largest holding 53 | ~7s |

Small samples draw from the common end of each option list, because twenty people
across the full spread of sizes and resolutions are all groups of one — true of the
draw, but not of real demand. Research is the expensive stage, so only the largest
groups go to the web; the rest are shown, waiting for more demand.
